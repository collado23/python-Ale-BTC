...
# --- 🚀 3. MOTOR V143 FRANCOTIRADOR (SOL, BNB, XRP) ---
def bot():
    ...
    print(f"🎯 V143 FRANCOTIRADOR | ${cap:.2f} | SOL-BNB-XRP")
    ...
    while True:
        ...
        try:
            for o in ops[:]:
                ...
                if (o['be'] and roi_n <= 0.2) or roi_n >= 3.5 or roi_n <= -2.5:
                    n_c = cap * (1 + (roi_n/100))
                    g_m(d=n_c); ops.remove(o); cap = n_c
                    print(f"\n✅ CIERRE EN {o['s']} | NETO: {roi_n:.2f}% | SALDO: ${cap:.2f}")

            # 3. ENTRADA (1 Operación, SOL/BNB/XRP)
            if len(ops) < 1:
                for m in monedas_target:
                    k = c.get_klines(symbol=m, interval='1m', limit=50)
                    cl = [float(x[4]) for x in k]
                    
                    # EMAs: 1 (Precio actual) y 32 (Tendencia)
                    e1 = cl[-1] 
                    e32 = sum(cl[-32:])/32 
                    ...
                    # Gatillo Bidireccional EMA 1/32
                    if v_act > v_apert and e1 > e32: # LONG
                        ops.append({'s':m,'l':'LONG','p':p_act,'x':5,'be':False})
                        print(f"\n🎯 DISPARO LONG: {m} (Cruce EMA 1/32)")
                        break
                    if v_act < v_apert and e1 < e32: # SHORT
                        ops.append({'s':m,'l':'SHORT','p':p_act,'x':5,'be':False})
                        print(f"\n🎯 DISPARO SHORT: {m} (Cruce EMA 1/32)")
                        break

            status = f"ROI: {roi_n:.2f}%" if len(ops) > 0 else "Buscando en SOL/BNB/XRP..."
