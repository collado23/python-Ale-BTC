...
def bot():
    ...
    print("🐊 MOTOR V146.6 | ROI REAL (SIN RESTAS) | 15X BINANCE")
        ...
        try:
            ...
            if op is None:
                ...
                for p in pos:
                    ...
                    if amt != 0:
                        ...
                        print(f"✅ ENGANCHADO: {p['symbol']} en {op['l']}")
                        break

            # 2. GESTIÓN (ROI LIMPIO)
            if op:
                p_act = float(c.futures_symbol_ticker(symbol=op['s'])['price'])
                if op['l'] == "LONG":
                    diff = (p_act - op['p']) / op['p']
                else:
                    diff = (op['p'] - p_act) / op['p']
                
                # ROI REAL SIN NINGUNA RESTA
                roi = (diff * 100 * op['x'])
                
                # 🔥 SALTO REAL 15X (Apenas toca 1.5% real)
            ...
            # 3. BUSCADOR
                ...
                for m in lista_m:
                    ...
                    tipo = None
                    if c_act > o_act: tipo = 'LONG'
                    elif c_act < o_act: tipo = 'SHORT'
            ...
            if ahora - u_p > 10:
                print("🔎..." if op is None else f"📊 {op['s']} {op['l']}: {roi:.2f}% | Piso: {op['piso']}%")
                u_p = ahora
