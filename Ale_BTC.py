...
def vigilante_bunker(c, sym, side, q, entry, palanca):
    ...
    pico = 0.0
    margen_pegado = 0.15 
    print(f"⚡ [AGRESIVO] Vigilando {sym} | Trail 0.15%")
    ...
    while True:
        try:
            ...
            if (pico >= 1.2 and roi <= piso) or (roi <= -4.0):
                ...
                print(f"🔥 CIERRE RÁPIDO EN {sym} | ROI: {roi:.2f}%")
...
def bot_quantum_v16_6():
    global contador_ops
    print("🚀 V16.6 | MODO AGRESIVO | ESPERA 10s")
    ...
    while True:
        try:
            ...
            # Adopción inmediata de cualquier moneda abierta
            for r in reales:
            ...
            # BUSCADOR AGRESIVO
            # Solo espera 10 segundos después de cerrar para buscar de nuevo
            if len(reales) == 0 and disp >= 7.80 and (time.time() - ultimo_cierre_tiempo > 10):
                for m in ['SOLUSDC', '1000PEPEUSDC']:
                    ...
                    side = None
                    # Condición simplificada: precio cruza la media rápida
                    if (cl[-1] > e9 > e27): side = SIDE_BUY
                    elif (cl[-1] < e9 < e27): side = SIDE_SELL
                    ...
                    if side:
                        ...
                        contador_ops += 1
                        print(f"🎯 ENTRADA RÁPIDA #{contador_ops} EN {m}")
                        break

            print(f"💰 WALLET: {total_w:.2f} | OPS: {contador_ops} | ACTIVAS: {len(reales)}")

        except Exception as e: time.sleep(5)
        time.sleep(10) # Escaneo más frecuente (cada 10s)
