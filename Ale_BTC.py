...
def vigilante_blindado(c, sym, side, q, entry, palanca, comision, stop_loss): 
    ...
    while sym in ops_activas:
        try:
            ...
            # CORRECCIÓN CLAVE: Binance usa markPrice, no mark_price
            ...
            if meta_ganada or stop_tocado:
                ...
                if sym in ops_activas: del ops_activas[s]
                break 
...
def bot_quantum_v13_final():
    ...
    # Monedas y configuración que pediste
    monedas = ['SOLUSDC', '1000PEPEUSDC', 'DOGEUSDC', 'ADAUSDC']
    palanca, stop_loss = 5, -4.0
    max_ops = 2 

    print(f"🚀 V13 TOTAL BLOCK | DOBLE POSICIÓN | 45% CAPITAL C/U")
    ...
    while True:
        try:
            ...
            # ARREGLO PARA QUE EL 45% SEA SOBRE EL TOTAL (9.74 USDC)
            ...
            print(f"\n💰 BALANCE: {total_w:.2f} | ACTIVAS: {len(reales)}/{max_ops}")
            ...
            # RADAR DE ENTRADA (Tu estrategia 9/27)
            if len(reales) < max_ops:
                for m in monedas:
                    ...
                    # 45% del total para asegurar la entrada de las 2
                    monto = total_w * 0.45 
