...
def bot():
    ...
    ops = []
    bloqueadas = {} # ⏱️ Registro de enfriamiento
    u_p = 0
    print("🐊 MOTOR V152.2 | ENFRIAMIENTO: 2 MINUTOS")
    ...
    while True:
        try:
            ...
            # 📊 2. GESTIÓN + SALTO 15X + ESCALADOR LARGO
            for o in ops[:]:
                ...
                if roi < (o['piso'] if o['be'] else sl_val):
                    ...
                    bloqueadas[o['s']] = ahora + 120 # ⏱️ BLOQUEO POR 2 MINUTOS (120 seg)
                    ops.remove(o)

            # 🎯 3. BUSCADOR (CON FILTRO DE 2 MINUTOS)
            if len(ops) == 0:
                for m in lista_m:
                    if m in bloqueadas and ahora < bloqueadas[m]:
                        continue
