...
def bot_quantum_trailing_105():
    ...
    print("🚀 ALE IA QUANTUM - TRAILING 1.05% ACTIVADO")
        ...
        try:
            ...
            if n_activas > 0:
                ...
                for activa in activas_info:
                    ...
                    nocional = q * m_p
                    capital_invertido = nocional / palanca
                    ...
                    # NUEVA LÓGICA: Arranca en 1.05%
                    piso = max_rois[sym] - 0.3 if max_rois[sym] >= 1.05 else -99.0

                    print(f"💰 CAPITAL USADO: {capital_invertido:.2f} USDC")
                    print(f"📈 {sym} ({side}) | ROI: {roi_pct:.2f}%")
                    print(f"🔝 MÁX: {max_rois[sym]:.2f}% | PISO CIERRE: {piso:.2f}%")
                    print("-" * 40)
            ...
            # --- BUSCADOR ---
                ...
                print(f"📡 BUSCANDO SEÑAL (Radar Secuencial) en {monedas}...", end='\r')
                for m in monedas:
                    ...
                    if (p_act > e9 > e27) or (p_act < e9 < e27):
                        ...
                        if (cant * p_act) >= 5.0:
                            ...
                            print(f"\n🎯 ENGANCHE: {m} | USANDO {monto_op:.2f} USDC")
                            break 
