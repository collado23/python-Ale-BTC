...
def vigilante_bunker(c, sym, side, q, entry, palanca, comision, stop_loss):
    ...
    pico = 0.0
    # Aumentamos el margen para evitar cierres instantáneos
    gatillo_trailing = 2.50 
    margen_pegado = 0.20
    
    print(f"🛡️ PROTEGIENDO: {sym} | Entrada: {entry}")
    ...
    while True:
        try:
            ...
            # Imprimir ROI para que lo veas en pantalla
            print(f"📊 {sym} | ROI: {roi:.2f}% | MAX: {pico:.2f}% | PISO: {piso:.2f}%")

            # Gatillos de salida ajustados para no perder por comisiones
...
def bot_quantum_v14_freno():
    api_key = os.getenv("API_KEY")
    api_secret = os.getenv("API_SECRET")
    c = Client(api_key, api_secret)
    c.API_URL = 'https://fapi.binance.com/fapi/v1'
    
    # SOLO DOS MONEDAS COMO PEDISTE
    monedas_radar = ['SOLUSDC', '1000PEPEUSDC']
    palanca = 5
    stop_loss = -8.0 # Más espacio para no ser sacado por ruido

    print("🚀 V14.3 - MODO DOS MONEDAS | FRENO DE SEGURIDAD ACTIVADO")
    ...
    while True:
        try:
            ...
            # Radar con protección de tiempo (espera 5 min entre trades)
            if len(simbolos_reales) < 2 and (time.time() - ultimo_cierre_tiempo > 300):
                for m in monedas_radar:
                    ...
                    monto = disp * 0.48 # Usar casi la mitad del saldo en cada una
                    decs = 0 if 'PEPE' in m else 1
                    cant = round((monto * palanca) / cl[-1], decs)
                    
                    if (cant * cl[-1]) >= 5.1:
                        c.futures_change_leverage(symbol=m, leverage=palanca)
                        c.futures_create_order(symbol=m, side=side_order, type=ORDER_TYPE_MARKET, quantity=cant)
                        print(f"🎯 ENTRADA PROGRAMADA EN {m}")
                        time.sleep(10)
                        break
