...
def bot_quantum_v13_final():
    ...
    print(f"🚀 V13 PANEL DE CONTROL ACTIVO | OBJETIVO: 1.20%")
    ...
    while True:
        try:
            ...
            for r in reales:
                ...
                # MOSTRAR ROI EN EL LOG PARA QUE VOS VEAS
                info = ops_activas.get(s, {})
                print(f"🔹 {s:10} | ROI: {info.get('roi',0.0):.2f}% | LADO: {'LONG' if float(r['positionAmt'])>0 else 'SHORT'}")
                ...
                for m in monedas:
                    ...
                    if es_long or es_short:
                        ...
                        if cant > 0:
                            ...
                            print(f"🎯 ENTRADA EN {m} DETECTADA")
                            time.sleep(5); break
            
            if not reales:
                print("⏳ Buscando señales de piquito...")

        except Exception as e:
            print(f"⚠️ Error: {e}")
            time.sleep(5)
        
        time.sleep(5) # Refresca el panel cada 5 segundos
